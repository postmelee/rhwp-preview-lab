...
def select_run(runs, sha, event, workflow_id):
    candidates = [r for r in runs if r['head_sha'] == sha and r['event'] == event and r['workflow_id'] == workflow_id]
    if not candidates:
    ...
    if newest['status'] != 'completed' or newest['conclusion'] != 'success':
        raise ValueError(f"CI not successful: {newest['id']} / {newest['status']} / {newest['conclusion']}")
    return newest
